...
namespace MDSolution
{
    public class clsGiongMia
    {
		...
		public void Save(OleDbConnection cn, OleDbTransaction trans)
		{
			...
			{
                //id = lddata.MDSolutionEntities.DBModule.GetNewID(typeof(GiongMia), "tbl_GiongMia", cn, trans);
				ID = MDSolutionEntities.DBModule.GetNewID(typeof(clsGiongMia), "tbl_GiongMia", cn, trans);
				//NgayTao = DateTime.Now;        
				//NgaySua = DateTime.Now;        
				// build SQL statement
                strSQL = "Insert into tbl_GiongMia" +
				"(ID,Ten,GhiChu,SoNgay,UuTien,GiaMia) Values(" +
					ID.ToString()+","+"N'" + MDSolutionEntities.DBModule.RefineString(Ten) + "'"+","+"N'" + MDSolutionEntities.DBModule.RefineString(GhiChu) + "'"+","+SoNgay.ToString()+","+UuTien.ToString()+","+GiaMia.ToString()+")";
			}
			else // edit object, we update old record in database
			{
                ...
                strSQL = "Update tbl_GiongMia set " +
                    "Ten=" + "N'" + MDSolutionEntities.DBModule.RefineString(Ten) + "'" + "," + "GhiChu=" + "N'" + MDSolutionEntities.DBModule.RefineString(GhiChu) + "'" + "," + "SoNgay=" + SoNgay.ToString() + "," + "UuTien=" + UuTien.ToString() + "," + "GiaMia=" + GiaMia.ToString() +
			...
			/*
			if( ID <= 0 )
				ID = long.Parse(MDSolutionEntities.DBModule.ExecuteQueryGetOneResult("SELECT Max(ID) FROM tbl_GiongMia", cn, trans));
				*/
		}
