...
namespace MDSolution
{
    public class clsPheCanh
    {
		...
		public void Save(OleDbConnection cn, OleDbTransaction trans)
		{
			...
			{
                //id = lddata.MDSolutionEntities.DBModule.GetNewID(typeof(PheCanh), "tbl_PheCanh", cn, trans);
				ID = MDSolutionEntities.DBModule.GetNewID(typeof(clsPheCanh), "tbl_PheCanh", cn, trans);
				//NgayTao = DateTime.Now;        
				//NgaySua = DateTime.Now;        
				// build SQL statement
                strSQL = "Insert into tbl_PheCanh" +
				"(ID,LyDoPheCanh,GhiChu) Values(" +
					ID.ToString()+","+"N'" + MDSolutionEntities.DBModule.RefineString(LyDoPheCanh) + "'"+","+"N'" + MDSolutionEntities.DBModule.RefineString(GhiChu) + "'"+")";
			}
			else // edit object, we update old record in database
			{
                ...
                strSQL = "Update tbl_PheCanh set " +
					"LyDoPheCanh="+"N'" + MDSolutionEntities.DBModule.RefineString(LyDoPheCanh) + "'"+","+"GhiChu="+"N'" + MDSolutionEntities.DBModule.RefineString(GhiChu) + "'"+
			...
			/*
			if( ID <= 0 )
				ID = long.Parse(MDSolutionEntities.DBModule.ExecuteQueryGetOneResult("SELECT Max(ID) FROM tbl_PheCanh", cn, trans));
				*/
		}
