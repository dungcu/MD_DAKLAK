...
{
    public partial class frmThanhToan2013_ThemMoi : Form
    {
        public frmThanhToan2013_ThemMoi()
        {
            InitializeComponent();
        }
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtSoDuConLai.Text.StartsWith("-"))
                {
                    MessageBox.Show("Số dư không đủ để thực hiện thanh toán! Vui lòng nhập tiền trả nợ.", "Thông báo");
                    return;
                }
                decimal TienTraNo = decimal.Parse(txtTienMatTraNo .Text.Replace(",", ""));
                if (TienTraNo > 0)
                {
                    string sql = "ThanhToan_NhapTienTraNo {0},{1},{2},{3},{4},{5},{6},N'{7}'";
                    sql = string.Format(sql, MDSolution.DACASUCO_App.VuTrongID, this.HopDongID, this.HopDongDTID, this.LaNoCu, this.SoPhieu, TienTraNo ,"'"+ this.NgayTra.ToString("MM/dd/yyyy") +"'", "");
                ...
                string sThanhToan_TruNoDauTu = "ThanhToan_TruNoDauTu {0},{1},{2},{3},{4},{5},{6},{7},{8}";
                sThanhToan_TruNoDauTu = string.Format(sThanhToan_TruNoDauTu, MDSolution.DACASUCO_App.VuTrongID, HopDongID, HopDongDTID, SoPhieu, TruGoc, TruLai, txtSoDuConLai.Text.Replace(",", ""), this.LaNoCu,"N'" +NgayTra.ToString("MM/dd/yyyy") + "'");
        ...
        public Boolean  Calc()
        {
            try
            {
                decimal dTienTraNo = decimal.Parse(txtTienMatTraNo.Text.Replace(",", ""));
                ...
                txtSoDuConLai.Text = (dDuHienTai+dTienTraNo  - dTienTruGoc - dTruLai).ToString("###,##0");
                return true;
            }
            catch
            {
        ...
        private void txtSoDuConLai_TextChanged(object sender, EventArgs e)
        {
            btnNhapTienTraNo.Visible = txtSoDuConLai.Text.StartsWith("-");            
        }

        private void btnNhapTienTraNo_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Người thanh toán muốn nhập số tiền:[" + txtSoDuConLai.Text.Substring(1) + "] để trừ nợ?", "Nhập tiền trả nợ", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
            {
                
                txtTienMatTraNo.Text = txtSoDuConLai.Text.Substring(1);
                Calc();
            }
        }
    }
